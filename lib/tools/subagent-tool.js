...
export function createSubagentTool(deps) {
  ...
  return {
    ...
    execute: async (_toolCallId, params) => {
      ...
      // await prepare 拿到 sessionPath；如果 prepare 失败，不影响 activeCount
      let childSessionPath;
      let run;
      try {
        const prepared = await deps.prepareIsolatedSession({
        ...
        childSessionPath = prepared.sessionPath;
        run = prepared.run;
      } catch (err) {
        store.fail(taskId, err.message || String(err));
        return {
          content: [{ type: "text", text: t("error.subagentFailed", { msg: err.message || String(err) }) }],
        };
      }

      // prepare 成功后才计入并发（run 的 finally 负责减）
      activeCount++;
