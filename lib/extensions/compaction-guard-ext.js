  ...
  return function (pi) {
    ...
    pi.on("session_before_compact", (event, ctx) => {
      try {
        ...
        const summarizeTokens = estimatePreparationTokens(preparation);
        const threshold = Math.floor(contextWindow * hardTruncateThreshold);

        if (summarizeTokens <= threshold) {
        ...
        if (!truncation) {
          ...
          log.warn(
            `[L3] hard-truncate unavailable: summarizeTokens=${summarizeTokens} ` +
        ...
        log.log(
          `[L3] preemptive hard-truncate: summarizeTokens=${summarizeTokens} ` +
